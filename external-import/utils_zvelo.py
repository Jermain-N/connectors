...
from src import log
import vault
from stix2 import Infrastructure, IPv4Address, IPv6Address, DomainName, Relationship, Indicator, Malware
...
def get_credentials(name: str):
    """
    Creates the required folder and file if not exists.
    Returns a dictionary representation of all items in the named section
    """
    vault.export_vault(name)
    cred = os.getenv(name)
    if cred:
        return cred
    else:
...
class ZveloIngest:
    ...
    def __init__(self,
        ...
        self.destination_filename = "data.ndjson"
    ...
    def _set_authentication_token(self):
        ...
        zvelo_client_id = get_credentials("ZVELO_CLIENT_ID")
        zvelo_client_secret = get_credentials("ZVELO_CLIENT_SECRET")
